...
test('latest Gametime build renders v034 tag decisions', async ({ page }) => {
  const errors = [];
  ...
  await expect(page.getByTestId('team-panel')).toContainText(/Matchup Builder|League|Start|Auto Offense|Next Camera|Call Screen|Hit Roll\/Pop|Force Tag Read/i);
  ...
  await expect(page.locator('#teamBadge')).toContainText(/Match started|stay-home|soft-tag|stunt|hard-tag/i);
  ...
  await expect(page.getByTestId('tag-decision-panel')).toContainText(/Ready|Stay|Tag|Rotate|Kickout|Payoff/);
  ...
  await expect(page.getByTestId('pass-feedback')).toContainText(/Complete kickout|Turnover - help trap|Stunted pass|Risk|Lane/);
  await page.keyboard.press('KeyJ');
  await expect(page.getByTestId('shot-feedback')).toContainText(/Make Chance|Paint|Kickout three|Contested Paint/);
...
test('mobile joystick responds without text selection artifacts in v034', async ({ browser }) => {
