...
test('latest Gametime build renders v033 roll pop reads', async ({ page }) => {
  ...
  await expect(page.getByTestId('roll-pop-panel')).toContainText(/Roll|Pop|Slip|Read live|Window/);
  await expect(page.getByTestId('action-pill')).toContainText(/read|window/i);
  ...
  await expect(page.getByTestId('action-pill')).toContainText(/jumps|contests|roll-pop/i);
